...
extension StateMachine {
    ...
    public func withState<T, E>(
        ...
        guard lock.lock(before: .now) else { return try failureBlock(context) }
        defer { lock.unlock() }
    ...
    public func withState<T, E>(
        ...
        _ block: (_ context: State.Context) throws(E) -> T,
        wrongState failureBlock: (_ context: State.Context) throws(E) -> T
    ) throws(E) -> T where State: Equatable {
        try withState(expecting: { $0 == expectedState }, block, wrongState: failureBlock)
    }
...
extension StateMachine {
    ...
    public func withState<T, E>(
        ...
        try await withState(expecting: { $0 == expectedState }, block, wrongState: failureBlock)
    }
