//
//  StateMachineStateStorageProtocol.swift
//  SwiftStateMachines • https://github.com/orchetect/swift-state-machines
//  © 2026 Steffan Andrews • Licensed under MIT License
//

public protocol StateMachineStateStorageProtocol<StateID>: AnyObject {
    associatedtype StateID: Hashable & Sendable

    var state: any StateMachineState<StateID> { get set }
    var resources: Any { get set }

    init<State: StateMachineState<StateID>>(state: consuming State, resources: consuming State.StateResources)
    init<State: StateMachineState<StateID>>(state: consuming State) where State.StateResources == Never
}
