...
struct StateMachine_StartStop_Static_Tests {
    ...
    func transitions() {
        ...
        // stop
        #expect(sm.transition(to: .stopped()) == .completed)
        #expect(sm.assertState(is: .stopped) == true)
        #expect(sm.transition(to: .stopped()) == .skipped) // already stopped
        ...
        // stop permanently
        #expect(sm.transition(to: .stoppedPermanently()) == .completed)
        #expect(sm.assertState(is: .stoppedPermanently) == true)

        // attempt to start; fails
        #expect(sm.transition(to: .started()) { 2 } == .failed) // not allowed; stopped permanently
        #expect(sm.assertState(is: .stoppedPermanently) == true) // unchanged

        // attempt to stop non-permanently; fails
        #expect(sm.transition(to: .stopped()) == .failed) // not allowed; stopped permanently
        #expect(sm.assertState(is: .stoppedPermanently) == true) // unchanged
    }
